...
QChar ComplexNumber::SEPARATOR = separator;
...
ComplexNumber::ComplexNumber(const QByteArray& arr)
{
	int index = arr.indexOf("-");

	a = arr.left(index).toDouble();
	b = arr.right(arr.length() - index - 1).toDouble();
}
...
QString& operator<<(QString& os, const ComplexNumber& number)
{
	os += QString::number(number.a);
	os += " + i( ";
	os += QString::number(number.b);
	os += " );";

	return os;
}

QByteArray& operator>>(QByteArray& arr, ComplexNumber& number)
{
	int p = arr.indexOf(ComplexNumber::SEPARATOR);

	if (p > 0)
	{
		number = ComplexNumber(arr.left(p));
